...
export class Doc {
  ...
  newPage() {
    if (this._measuring) { this.y = this.margin; return; }
    this._endPage(); this._startPage();
  }
  get usableHeight() { return this.h - this.margin * 2; }
  ...
  /**
   * How tall would `fn` draw? Runs it with output and pagination suppressed,
   * then rewinds. Lets a caller keep a block whole by asking first, instead of
   * duplicating the block's layout arithmetic in a second place.
   */
  measure(fn) {
    const y0 = this.y, ops = this.ops, pageCount = this.pages.length;
    this._measuring = true; this.ops = [];
    let h = 0;
    try { fn(); h = this.y - y0; }
    finally {
      this._measuring = false;
      this.y = y0; this.ops = ops; this.pages.length = pageCount;
    }
    return h;
  }
  /** Place `fn` whole: break first if it will not fit and could fit alone. */
  keepTogether(fn) {
    const h = this.measure(fn);
    if (h <= this.usableHeight) this.room(h);
    fn();
  }
  space(n) { this.y += n; }
